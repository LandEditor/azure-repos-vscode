...
describe("GitContext", function () {
	...
	it("should verify repository with an empty origin remote", function () {
		...
		assert.equal(gc.RemoteUrl, undefined);
		assert.equal(
			gc.RepositoryParentFolder,
			path.join(__dirname, TEST_REPOS_FOLDER, repoName)
		);
	...
	it("should verify TeamServices origin remote", function () {
		...
		assert.equal(
			gc.RemoteUrl,
			"https://account.visualstudio.com/DefaultCollection/teamproject/_git/gitrepo"
		);
		assert.equal(
			gc.RepositoryParentFolder,
			path.join(__dirname, TEST_REPOS_FOLDER, repoName)
	...
	it("should verify TeamServices origin remote cloned with old-style ssh url", function () {
		...
		assert.equal(
			gc.RemoteUrl,
			"https://account.visualstudio.com/DefaultCollection/_git/repository"
		);
		assert.equal(
			gc.RepositoryParentFolder,
			path.join(__dirname, TEST_REPOS_FOLDER, repoName)
	...
	it("should verify TeamFoundationServer origin remote", function () {
		...
		assert.equal(
			gc.RemoteUrl,
			"http://devmachine:8080/tfs/DefaultCollection/_git/GitAgile"
		);
		assert.equal(
			gc.RepositoryParentFolder,
			path.join(__dirname, TEST_REPOS_FOLDER, repoName)
	...
	it("should verify TeamFoundationServer origin remote cloned with ssh", function () {
		...
		assert.equal(
			gc.RemoteUrl,
			"ssh://devmachine:22/tfs/DefaultCollection/_git/GitJava"
		);
		assert.equal(
			gc.RepositoryParentFolder,
			path.join(__dirname, TEST_REPOS_FOLDER, repoName)
