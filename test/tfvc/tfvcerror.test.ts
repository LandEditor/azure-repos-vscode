...
describe("Tfvc-Error", function () {
	...
	it("should verify CreateInvalidStateError", function () {
		...
		assert.equal(
			error.message,
			"The TFVC SCMProvider is in an invalid state for this action."
		);
		assert.equal(error.stderr, undefined);
		assert.equal(error.stdout, undefined);
		assert.equal(error.tfvcCommand, undefined);
	...
	it("should verify CreateUnknownError", function () {
		const error: TfvcError = TfvcError.CreateUnknownError({
			name: "err1",
	...
	it("should verify toString", function () {
		...
		assert.equal(
			error.toString(),
			"error1 message Details: exitCode: 11, errorCode: TfvcMinVersionWarning, command: command1, stdout: standard output, stderr: standard error Stack: here; then there"
		);
	});
});
