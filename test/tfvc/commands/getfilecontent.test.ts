...
describe("Tfvc-GetFileContentCommand", function () {
	...
	it("should verify constructor - undefined args", function () {
		assert.throws(
			() => new GetFileContent(undefined, undefined),
			TfvcError,
	...
	it("should verify GetArguments + versionSpec", function () {
		const localPath: string =
			"/usr/alias/repos/Tfvc.L2VSCodeExtension.RC/README.md";
		const cmd: GetFileContent = new GetFileContent(
			undefined,
			localPath,
			"42"
		);
		...
		assert.equal(
			cmd.GetArguments().GetArgumentsForDisplay(),
			"print -noprompt " + localPath + " -version:42"
		);
	});
	...
	it("should verify GetExeArguments + versionSpec", function () {
		...
		assert.equal(
			cmd.GetExeArguments().GetArgumentsForDisplay(),
			"view -noprompt " + localPath + " -version:42"
		);
	});
