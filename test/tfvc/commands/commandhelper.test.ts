...
describe("Tfvc-CommandHelper", function () {
	...
	it("should verify HasError - has errors", function () {
		...
		assert.equal(
			CommandHelper.HasError(result, "Something bad happened!"),
			true
		);
		assert.equal(
			CommandHelper.HasError(result, "Something bad happened"),
	...
	it("should verify ProcessErrors - auth failed", function () {
		...
		try {
			...
			assert.equal(
				err.tfvcErrorCode,
				TfvcErrorCodes.AuthenticationFailed
			);
			assert.isTrue(err.message.startsWith(Strings.TfExecFailedError));
		}
	});
	...
	it("should verify ProcessErrors - no java", function () {
		...
		try {
			...
			assert.isTrue(
				err.message.startsWith(Strings.TfInitializeFailureError)
			);
		}
	});
	...
	it("should verify ProcessErrors - Server workspace detection (TF30063)", function () {
		...
		try {
			...
			assert.equal(
				err.tfvcErrorCode,
				TfvcErrorCodes.NotAuthorizedToAccess
			);
	...
	it("should verify GetNewLineCharacter", async function () {
		...
		assert.equal(
			CommandHelper.GetNewLineCharacter(
				"blah blah\nblah blah\n\rblah\nblah"
			),
			"\n"
		);
		...
		assert.equal(
			CommandHelper.GetNewLineCharacter(
				"blah /r/nblah blah/n blah\r blah\r blah"
			),
			"\n"
		);
	});
	...
	it("should verify GetFilePath", async function () {
		assert.equal(
			CommandHelper.GetFilePath(undefined, undefined, undefined),
			undefined
		);
		assert.equal(
			CommandHelper.GetFilePath("/path/folder", "file.txt", undefined),
		...
		assert.equal(
			CommandHelper.GetFilePath(undefined, "file.txt", undefined),
			"file.txt"
		);
		assert.equal(
			CommandHelper.GetFilePath("/path/folder", undefined, undefined),
			"/path/folder"
		);
		assert.equal(
			CommandHelper.GetFilePath("/path/folder:", undefined, undefined),
		...
		assert.equal(
			CommandHelper.GetFilePath("folder:", "file.txt", undefined),
			path.join("folder", "file.txt")
		);
		assert.equal(
			CommandHelper.GetFilePath("folder:", "file.txt", "/root"),
			path.join("/root/folder", "file.txt")
		);
		assert.equal(
			CommandHelper.GetFilePath(undefined, "file.txt", "/root"),
