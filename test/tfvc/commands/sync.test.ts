...
describe("Tfvc-SyncCommand", function () {
	...
	it("should verify constructor - undefined args", function () {
		assert.throws(
			() => new Sync(undefined, undefined, false),
			TfvcError,
	...
	it("should verify arguments", function () {
		...
		assert.equal(
			cmd.GetArguments().GetArgumentsForDisplay(),
			"get -noprompt -nosummary " + localPaths[0]
		);
	});
	...
	it("should verify getExeArguments", function () {
		...
		assert.equal(
			cmd.GetExeArguments().GetArgumentsForDisplay(),
			"get -noprompt -nosummary " + localPaths[0]
		);
	});
	...
	it("should verify getExeArguments with context and recursive", function () {
		...
		assert.equal(
			cmd.GetExeArguments().GetArgumentsForDisplay(),
			"get -noprompt ******** -nosummary " + localPaths[0] + " -recursive"
		);
	});
	...
	it("should verify parse output - single file edit - no errors", async function () {
		...
		assert.equal(
			results.itemResults[0].itemPath,
			path.join("/usr/alias/repo1/test", "test1.txt")
		);
	});
	...
	it("should verify parse output - single file add - spaces - no errors", async function () {
		...
		assert.equal(
			results.itemResults[0].itemPath,
			path.join("/usr/alias/repo 1/test", "test 1.txt")
		);
	});
	...
	it("should verify parse output - multiple files - with conflict", async function () {
		...
		assert.equal(
			results.itemResults[0].itemPath,
			path.join("/usr/alias/repo1", "addFold")
		);
		assert.equal(results.itemResults[1].syncType, SyncType.New);
		assert.equal(
			results.itemResults[1].itemPath,
			path.join("/usr/alias/repo1", "addFold-branch")
		);
		assert.equal(results.itemResults[2].syncType, SyncType.Updated);
		assert.equal(
			results.itemResults[2].itemPath,
			path.join("/usr/alias/repo1/addFold-branch", "testHereRename.txt")
		);
		assert.equal(results.itemResults[3].syncType, SyncType.New);
		assert.equal(
			results.itemResults[3].itemPath,
			path.join("/usr/alias/repo1/addFold", "testHere3")
		);
		assert.equal(results.itemResults[4].syncType, SyncType.Updated);
		assert.equal(
			results.itemResults[4].itemPath,
			path.join("/usr/alias/repo1/addFold", "testHereRename7.txt")
		);
		assert.equal(results.itemResults[5].syncType, SyncType.Deleted);
		assert.equal(
			results.itemResults[5].itemPath,
			path.join("/usr/alias/repo1/last", "Rename2.txt")
		);
		assert.equal(results.itemResults[6].syncType, SyncType.Updated);
		assert.equal(
			results.itemResults[6].itemPath,
			path.join("/usr/alias/repo1/last", "test3.txt")
		);
		assert.equal(results.itemResults[7].syncType, SyncType.New);
		assert.equal(
			results.itemResults[7].itemPath,
			path.join("/usr/alias/repo1/last", "TestAdd.txt")
		);
		...
		assert.equal(
			results.itemResults[8].message,
			"Unable to perform the get operation because you have a conflicting rename, edit"
		);
	});
	...
	it("should verify parse output - errors - exit code 1", async function () {
		...
		const executionResult: IExecutionResult = {
			...
			stderr:
				"Conflict new.txt - Unable to perform the get operation because you have a conflicting edit\n" +
		...
		assert.equal(
			results.itemResults[0].message,
			"Unable to perform the get operation because you have a conflicting edit"
		);
		assert.equal(results.itemResults[1].syncType, SyncType.Error);
		assert.equal(results.itemResults[1].itemPath, "new4.txt");
		assert.equal(
			results.itemResults[1].message,
			"Unable to perform the get operation because you have a conflicting rename (to be moved from /path/new5.txt)"
		);
		assert.equal(results.itemResults[2].syncType, SyncType.Warning);
		assert.equal(results.itemResults[2].itemPath, "new111.txt");
		assert.equal(
			results.itemResults[2].message,
