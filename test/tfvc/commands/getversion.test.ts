...
describe("Tfvc-GetVersionCommand", function () {
	...
	it("should verify parse output - no version", async function () {
		...
		try {
			...
			assert.equal(
				err.tfvcErrorCode,
				TfvcErrorCodes.NotAnEnuTfCommandLine
			);
			assert.isTrue(
				err.message.startsWith(Strings.NotAnEnuTfCommandLine)
			);
			threw = true;
		} finally {
			assert.isTrue(threw);
	...
	it("should verify parse output - java object heap error", async function () {
		...
		try {
			...
			assert.isTrue(
				threw,
				"Checking for Java object heap error did not throw an error."
			);
		}
	});
	...
	it("should verify parse EXE output - Spanish version", async function () {
		...
		try {
			...
			assert.isTrue(
				threw,
				"Checking Spanish version did not throw an error."
			);
		}
	});
	...
	it("should verify parse EXE output - French version", async function () {
		...
		try {
			...
			assert.isTrue(
				threw,
				"Checking French version did not throw an error."
			);
		}
	});
