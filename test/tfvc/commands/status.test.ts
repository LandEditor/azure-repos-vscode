...
describe("Tfvc-StatusCommand", function () {
	...
	it("should verify arguments", function () {
		...
		assert.equal(
			...
			"status -noprompt -format:xml -recursive " + localPaths[0]
		);
	});
	...
	it("should verify Exe arguments", function () {
		...
		assert.equal(
			...
			"status -noprompt -format:detailed -recursive " + localPaths[0]
		);
	});
	...
	it("should verify arguments - no paths", function () {
		...
		assert.equal(
			cmd.GetArguments().GetArgumentsForDisplay(),
			"status -noprompt -format:xml -recursive"
		);
	});
	...
	it("should verify Exe arguments - no paths", function () {
		...
		assert.equal(
			cmd.GetExeArguments().GetArgumentsForDisplay(),
			"status -noprompt -format:detailed -recursive"
		);
	});
	...
	it("should verify arguments with context", function () {
		...
		assert.equal(
			cmd.GetArguments().GetArgumentsForDisplay(),
			"status -noprompt -collection:" +
				collectionUrl +
				" ******** -format:xml -recursive " +
				localPaths[0]
		);
	});
	...
	it("should verify parse output - valid json", async function () {
     ...
     <pending-change server-item="$/tfsTest_01/test.txt" version="0" owner="jason" date="2016-07-13T12:36:51.060-0400" lock="none" change-type="add" workspace="Folder1_00" computer="JPRICKET-DEV2" local-item="/tmp/test/test.txt"/>
		...
		assert.equal(
			changes[0].localItem,
			"/tmp/tfsTest03_44/Folder333/DemandEquals_renamed.java"
		);
		assert.equal(changes[0].lock, "none");
		assert.equal(changes[0].owner, "NORTHAMERICA\\jpricket");
		assert.equal(
			changes[0].serverItem,
			"$/tfsTest_03/Folder333/DemandEquals_renamed.java"
		);
		assert.equal(
			changes[0].sourceItem,
			"$/tfsTest_03/Folder333/DemandEquals.java"
		);
	...
	it("should verify parse Exe output - pending changes only - no errors", async function () {
		...
		assert.equal(
			changes[0].date,
			"Wednesday, February 22, 2017 1:47:26 PM"
		);
		assert.isFalse(changes[0].isCandidate);
		assert.equal(
			changes[0].localItem,
			"C:\\repos\\TfExe.Tfvc.L2VSCodeExtension.RC.TFS\\README.md"
		);
		assert.equal(changes[0].lock, "none");
	...
	it("should verify parse Exe output - pending and detected changes - no errors", async function () {
		...
		assert.equal(
			changes[1].localItem,
			"C:\\repos\\TfExe.Tfvc.L2VSCodeExtension.RC.TFS\\therightstuff.txt"
		);
		assert.equal(changes[1].lock, "none");
	...
	it("should verify parse Exe output - pending rename only - no errors", async function () {
		...
		assert.equal(
			changes[0].localItem,
			"C:\\repos\\TfExe.Tfvc.L2VSCodeExtension.RC.TFS\\READU.md"
		);
		assert.equal(changes[0].lock, "none");
		assert.equal(changes[0].owner, "Jeff Young (TFS)");
