...
describe("Tfvc-CheckinCommand", function () {
	...
	it("should verify constructor - undefined args", function () {
		assert.throws(
			() => new Checkin(undefined, undefined),
			TfvcError,
	...
	it("should verify arguments with comment", function () {
		...
		assert.equal(
			...
			"checkin -noprompt -collection:" +
				...
				" ******** " +
				files[0] +
				" -comment:a comment that has multiple lines"
		);
	});
	...
	it("should verify arguments with all params", function () {
		...
		assert.equal(
			...
			"checkin -noprompt -collection:" +
				...
				" ******** " +
				files[0] +
				" -comment:a comment -associate:1,2,3"
		);
	});
	...
	it("should verify Exe arguments with all params", function () {
		...
		assert.equal(
			cmd.GetExeArguments().GetArgumentsForDisplay(),
			"checkin -noprompt ******** " + files[0] + " -comment:a comment"
		);
	});
