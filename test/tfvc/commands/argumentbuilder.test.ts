...
describe("Tfvc-ArgumentBuilder", function () {
	...
	it("should verify constructor with context - user and domain", function () {
		context.CredentialInfo = new CredentialInfo(
			user,
			pass,
			"domain",
			"workstation"
	...
	it("should verify BuildCommandLine with context", function () {
		...
		const builder: ArgumentBuilder = new ArgumentBuilder(cmd, context);
		assert.equal(
			builder.BuildCommandLine().trim(),
			"mycmd -noprompt -collection:" +
				collectionUrl +
				" -login:" +
				user +
				"," +
	...
	it("should verify BuildCommandLine with spaces in options", function () {
		...
		assert.equal(
			builder.BuildCommandLine().trim(),
			'mycmd -noprompt "/path/with a space/file.txt" "-option with space" "-option with space:/path/with a space/file.txt"'
		);
	});
	...
	it("should verify BuildCommandLine with spaces in some", function () {
		...
		assert.equal(
			builder.BuildCommandLine().trim(),
			'mycmd -noprompt "/path/with a space/file.txt" -option "-option:/path/with a space/file.txt"'
		);
	});

	it("should verify interface implemented", function () {
		const cmd: string = "mycmd";
		const argProvider: IArgumentProvider = new ArgumentBuilder(
			cmd,
			context
		);
		// GetCommand
		...
		assert.equal(
			argProvider.GetArgumentsForDisplay(),
			"mycmd -noprompt -collection:" + collectionUrl + " ********"
		);
		// GetCommandLine
		assert.equal(
			argProvider.GetCommandLine(),
			"mycmd -noprompt -collection:" +
				collectionUrl +
				" -login:" +
				user +
