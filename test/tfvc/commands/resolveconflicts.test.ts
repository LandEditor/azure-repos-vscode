...
describe("Tfvc-ResolveConflictsCommand", function () {
	...
	it("should verify constructor - undefined args", function () {
		assert.throws(
			() => new ResolveConflicts(undefined, undefined, undefined),
			TfvcError,
	...
	it("should verify GetOptions", function () {
		const localPaths: string[] = ["/usr/alias/repo1/file.txt"];
		const cmd: ResolveConflicts = new ResolveConflicts(
			undefined,
			localPaths,
	...
	it("should verify arguments", function () {
		...
		assert.equal(
			...
			"resolve -noprompt " + localPaths[0] + " -auto:KeepYours"
		);
	});
	...
	it("should verify GetExeArguments with TakeTheirs", function () {
		...
		assert.equal(
			cmd.GetExeArguments().GetArgumentsForDisplay(),
			"resolve -noprompt ******** " + localPaths[0] + " -auto:TakeTheirs"
		);
	});
	...
	it("should verify parse output - no errors", async function () {
		const localPaths: string[] = [
			"/usr/alias/repo1/file.txt",
			"/usr/alias/repo1/file2.txt",
		];
		const cmd: ResolveConflicts = new ResolveConflicts(
			undefined,
			localPaths,
		...
		const executionResult: IExecutionResult = {
			exitCode: 0,
			stdout:
				"Resolved /usr/alias/repo1/file.txt as KeepYours\n" +
