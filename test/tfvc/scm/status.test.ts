...
describe("Tfvc-Version", function () {
	...
	it("should verify GetStatuses - single value", function () {
		...
		expect(GetStatuses("source rename")).to.have.same.members([
			Status.RENAME,
		]);
	...
	it("should verify GetStatuses - multiple values", function () {
		expect(GetStatuses("add, edit")).to.have.same.members([
			Status.ADD,
			Status.EDIT,
		]);
		expect(GetStatuses("branch    ,    lock")).to.have.same.members([
			Status.BRANCH,
			Status.LOCK,
		]);
		expect(GetStatuses("     delete    ,merge   ")).to.have.same.members([
			Status.DELETE,
			Status.MERGE,
		]);
	});
	...
	it("should verify GetStatuses - multiple values with unknown", function () {
		expect(GetStatuses("add, unk, edit")).to.have.same.members([
			Status.ADD,
			Status.UNKNOWN,
			Status.EDIT,
		]);
		expect(GetStatuses("unk  , branch    ,    lock")).to.have.same.members([
			Status.UNKNOWN,
			Status.BRANCH,
			Status.LOCK,
		]);
		expect(
			GetStatuses("     delete    ,merge,    unk   ")
		).to.have.same.members([Status.DELETE, Status.MERGE, Status.UNKNOWN]);
	});
});
