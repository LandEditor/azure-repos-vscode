...
describe("GitVcService", function () {
	...
	it("should verify GetCreatePullRequestUrl", function () {
		const url: string =
			"https://account.visualstudio.com/DefaultCollection/project";
		const branch: string = "branch";

		assert.equal(
			GitVcService.GetCreatePullRequestUrl(url, branch),
			url + "/pullrequests#_a=createnew&sourceRef=" + branch
		);
	});
	...
	it("should verify GetFileBlameUrl", function () {
		...
		assert.equal(
			GitVcService.GetFileBlameUrl(url, file, branch),
			url + "#path=" + file + "&version=GB" + branch + "&annotate=true"
		);
	});
	...
	it("should verify GetFileHistoryUrl", function () {
		...
		assert.equal(
			GitVcService.GetFileHistoryUrl(url, file, branch),
			url + "#path=" + file + "&version=GB" + branch + "&_a=history"
		);
	});

	it("should verify GetPullRequestDiscussionUrl", function () {
		const repositoryUrl: string =
			"https://account.visualstudio.com/DefaultCollection/_git/project";
		const id: string = "42";

		assert.equal(
			GitVcService.GetPullRequestDiscussionUrl(repositoryUrl, id),
			repositoryUrl + "/pullrequest/" + id + "?view=discussion"
		);
	});
	...
	it("should verify GetPullRequestsUrl", function () {
		...
		assert.equal(
			GitVcService.GetPullRequestsUrl(repositoryUrl),
	...
	it("should verify GetPullRequestDiscussionUrl", function () {
		...
		assert.equal(
			GitVcService.GetRepositoryHistoryUrl(repositoryUrl, branch),
			repositoryUrl +
				"/history" +
				"?itemVersion=GB" +
				branch +
	...
	it("should verify failed pull request score", function () {
		...
		assert.equal(
			GitVcService.GetPullRequestScore(pullRequest),
