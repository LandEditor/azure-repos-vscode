...
describe("BuildService", function () {
	...
	it("should verify GetBuildDefinitionUrl", function () {
		...
		assert.equal(
			BuildService.GetBuildDefinitionUrl(url, arg),
			url + "/_build#_a=completed&definitionId=" + arg
		);
	});
	...
	it("should verify GetBuildSummaryUrl", function () {
		...
		assert.equal(
			BuildService.GetBuildSummaryUrl(url, arg),
			url + "/_build/index?buildId=" + arg + "&_a=summary"
		);
	});
