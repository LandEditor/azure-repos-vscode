...
describe("WorkItemTrackingService", function () {
	...
	it("should verify GetEditWorkItemUrl", function () {
		...
		assert.equal(
			WorkItemTrackingService.GetEditWorkItemUrl(url, id),
			url + "/_workitems" + "/edit/" + id
		);
	});
	...
	it("should verify GetNewWorkItemUrl with only url and issueType", function () {
		...
		assert.equal(
			WorkItemTrackingService.GetNewWorkItemUrl(url, issueType),
			url + "/_workitems" + "/create/" + issueType
		);
	});
	...
	it("should verify GetNewWorkItemUrl with url, issueType and title", function () {
		...
		assert.equal(
			WorkItemTrackingService.GetNewWorkItemUrl(url, issueType, title),
			url +
				"/_workitems" +
				"/create/" +
				issueType +
	...
	it("should verify GetMyQueryResultsUrl with url and queryName", function () {
		...
		assert.equal(
			WorkItemTrackingService.GetMyQueryResultsUrl(
				url,
				"My Queries",
				queryName
			),
	...
	it("should verify GetWorkItemsBaseUrl with url", function () {
		...
		assert.equal(
			WorkItemTrackingService.GetWorkItemsBaseUrl(url),
			url + "/_workitems"
		);
	});
});
