...
describe("UrlBuilder", function () {
	...
	it("should ensure baseUrl with trailing slash and multiple args returns the proper url", function () {
		...
		const result: string = UrlBuilder.Join(
			url,
			"_build",
			"index",
			"buildId=42"
	...
	it("should ensure AddQueryParams supports baseUrl with trailing slash and single query params", function () {
		...
		assert.equal(
			"http://xplatalm.visualstudio.com/index?buildId=42",
			result
		);
	});
	...
	it("should ensure AddQueryParams supports multiple query parameters", function () {
		...
		let result: string = UrlBuilder.AddQueryParams(
			url,
			"buildId=42",
			"whatever=andever",
			"foo=bar"
		...
		//Test with leading "&"
		result = UrlBuilder.AddQueryParams(
			url,
			"buildId=42",
			"whatever=andever",
			"foo=bar"
