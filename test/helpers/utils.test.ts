...
describe("Utils", function () {
	...
	it("should verify FindGitFolder with subfolder", function () {
		...
		const repoPath: string = path.join(
			__dirname,
			TEST_REPOS_FOLDER,
			repoName,
			"folder",
		...
		const actualRepoPath: string = Utils.FindGitFolder(
			repoPath,
			DOT_GIT_FOLDER
		);
		...
		assert.equal(
			actualRepoPath,
			path.join(__dirname, TEST_REPOS_FOLDER, repoName, DOT_GIT_FOLDER)
		);
	});
	...
	it("should verify GetBuildResultIcon with all values", function () {
		expect(Utils.GetBuildResultIcon(BuildResult.Succeeded)).to.equal(
			"octicon-check"
		);
		expect(Utils.GetBuildResultIcon(BuildResult.Canceled)).to.equal(
			"octicon-alert"
		);
		expect(Utils.GetBuildResultIcon(BuildResult.Failed)).to.equal(
			"octicon-stop"
		);
		expect(
			Utils.GetBuildResultIcon(BuildResult.PartiallySucceeded)
		).to.equal("octicon-alert");
		expect(Utils.GetBuildResultIcon(BuildResult.None)).to.equal(
			"octicon-question"
		);
		expect(Utils.GetBuildResultIcon(undefined)).to.equal(
			"octicon-question"
		);
	});
