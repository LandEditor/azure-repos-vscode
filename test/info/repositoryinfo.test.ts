...
describe("RepositoryInfo", function () {
	...
	it("should verify host, account and isTeamFoundationServer for valid remoteUrl", function () {
		const url: string =
			"http://jeyou-dev00000:8080/tfs/DefaultCollection/_git/GitAgile";
	...
	it("should verify valid values in repositoryInfo to RepositoryInfo constructor", function () {
		const repositoryInfo: any = {
			...
			"repository": {
				...
				"remoteUrl":
					"http://server:8080/tfs/DefaultCollection/teamproject/_git/repositoryName",
			},
		};
		...
		assert.equal(
			repoInfo.CollectionId,
			"d543db53-9479-46c1-9d33-2cb9cb76f622"
		);
		assert.equal(repoInfo.CollectionName, "DefaultCollection");
		assert.equal(
			repoInfo.CollectionUrl,
		...
		assert.equal(
			...
			"23344766-d9c7-4661-856d-b2096753c5e3"
		);
		...
		assert.equal(
			repoInfo.TeamProjectUrl,
			"http://server:8080/tfs/DefaultCollection/teamproject"
		);
	});
	...
	it("should verify undefined remoteUrl", function () {
		assert.throws(() => {
			new RepositoryInfo(undefined);
		});
	});

	it("should verify host, account and isTeamServices for valid remoteUrl", function () {
		const url: string =
			"https://account.visualstudio.com/DefaultCollection/teamproject/_git/repositoryName";
		const repoInfo: RepositoryInfo = new RepositoryInfo(url);
	...
	it("should verify host, account and isTeamServices for valid azure remoteUrl", function () {
		const url: string =
			"https://test.azure.com/account/teamproject/_git/repositoryName";
		const repoInfo: RepositoryInfo = new RepositoryInfo(url);
	...
	it("should verify host, account and isTeamServices for valid remoteUrl - limited refs - full", function () {
		const url: string =
			"https://account.visualstudio.com/DefaultCollection/teamproject/_git/_full/repositoryName";
		const repoInfo: RepositoryInfo = new RepositoryInfo(url);
		assert.equal(repoInfo.Host, "account.visualstudio.com");
		assert.equal(repoInfo.Account, "account");
		assert.equal(repoInfo.Protocol, "https:");
	...
	it("should verify host, account and isTeamServices for valid azure remoteUrl - limited refs - full", function () {
		const url: string =
			"https://test.azure.com/account/teamproject/_git/_full/repositoryName";
		const repoInfo: RepositoryInfo = new RepositoryInfo(url);
		assert.equal(repoInfo.Host, "test.azure.com");
		assert.equal(repoInfo.Account, "account");
		assert.equal(repoInfo.Protocol, "https:");
	...
	it("should verify host, account and isTeamServices for valid remoteUrl - limited refs - optimized", function () {
		const url: string =
			"https://account.visualstudio.com/DefaultCollection/teamproject/_git/_optimized/repositoryName";
		const repoInfo: RepositoryInfo = new RepositoryInfo(url);
		assert.equal(repoInfo.Host, "account.visualstudio.com");
		assert.equal(repoInfo.Account, "account");
		assert.equal(repoInfo.Protocol, "https:");
	...
	it("should verify host, account and isTeamServices for valid azure remoteUrl - limited refs - optimized", function () {
		const url: string =
			"https://test.azure.com/account/teamproject/_git/_optimized/repositoryName";
		const repoInfo: RepositoryInfo = new RepositoryInfo(url);
		assert.equal(repoInfo.Host, "test.azure.com");
		assert.equal(repoInfo.Account, "account");
		assert.equal(repoInfo.Protocol, "https:");
	...
	it("should verify valid values in repositoryInfo to RepositoryInfo constructor", function () {
		let repoInfo: RepositoryInfo = new RepositoryInfo(
			"https://account.visualstudio.com/DefaultCollection/teamproject/_git/repositoryName"
		);
		assert.equal(repoInfo.Host, "account.visualstudio.com");
		assert.equal(repoInfo.Account, "account");
		...
		const repositoryInfo: any = {
			...
			"repository": {
				...
				"remoteUrl":
					"https://account.visualstudio.com/teamproject/_git/repositoryName",
			},
		};
		...
		assert.equal(
			repoInfo.CollectionId,
			"5e082e28-e8b2-4314-9200-629619e91098"
		);
		...
		assert.equal(
			repoInfo.RepositoryId,
			"cc015c05-de20-4e3f-b3bc-3662b6bc0e42"
		);
		assert.equal(repoInfo.RepositoryName, "repositoryName");
		assert.equal(
			repoInfo.RepositoryUrl,
		...
		assert.equal(
			repoInfo.TeamProjectUrl,
			"https://account.visualstudio.com/teamproject"
		);
	});
	...
	it("should verify valid values in repositoryInfo to RepositoryInfo constructor for azure", function () {
		...
		assert.equal(
			repoInfo.TeamProjectUrl,
			"https://test.azure.com/account/teamproject"
		);
	});
	...
	it("should verify valid values in repositoryInfo to RepositoryInfo constructor - limited refs - full", function () {
		...
		const repositoryInfo: any = {
			...
			"repository": {
				...
				"remoteUrl":
					"https://account.visualstudio.com/teamproject/_git/_full/repositoryName",
			},
		};
		repoInfo = new RepositoryInfo(repositoryInfo);
		assert.equal(repoInfo.Host, "account.visualstudio.com");
	...
	it("should verify valid values in repositoryInfo to RepositoryInfo constructor - limited refs - optimized", function () {
		...
		const repositoryInfo: any = {
			...
			"repository": {
				...
				"remoteUrl":
					"https://account.visualstudio.com/teamproject/_git/_optimized/repositoryName",
			},
		};
		repoInfo = new RepositoryInfo(repositoryInfo);
		assert.equal(repoInfo.Host, "account.visualstudio.com");
