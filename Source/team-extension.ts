...
export class TeamExtension {
	...
	private async requestPersonalAccessToken(): Promise<string> {
		...
		choices.push({
			...
			description: undefined,
			id: DeviceFlowConstants.DeviceFlowOption,
		});

		const choice: BaseQuickPickItem = await window.showQuickPick(choices, {
		...
		if (choice) {
			if (choice.id === DeviceFlowConstants.ManualOption) {
				...
				const tokenOptions: IDeviceFlowTokenOptions = {
					tokenDescription: `Azure Repos VSCode extension: ${
				...
				const value: string = await window.showInputBox({
					...
					prompt: `${Strings.DeviceFlowCopyCode} (${details.VerificationUrl})`,
					...
					const token: string = await window.withProgress(
						...
						async () => {
							const accessToken: string =
								await dfa.WaitForPersonalAccessToken();
	...
	public async Signin() {
		...
		if (
			this._manager.ServerContext !== undefined &&
			...
			if (
				...
				this._manager.ServerContext.RepoInfo.IsTeamServices === true &&
				!this._signingIn
			) {
				...
				try {
					...
					if (err.message) {
						...
						if (
							err.message.indexOf("Request canceled by user") ===
							-1
						) {
							Telemetry.SendEvent(
	...
	public OpenFileHistory(): void {
		if (this._manager.EnsureInitialized(RepositoryType.ANY)) {
			if (
				this._manager.RepoContext.Type === RepositoryType.GIT &&
				this._gitClient
			) {
