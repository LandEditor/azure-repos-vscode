...
export class ExtensionManager implements Disposable {
	...
	public EnsureInitialized(expectedType: RepositoryType): boolean {
		...
		if (
			expectedType !== this._repoContext.Type &&
		...
		if (
			(expectedType === RepositoryType.TFVC ||
				expectedType === RepositoryType.ANY) &&
	...
	public ReportError(err: Error, message: string, showToUser = false): void {
		...
		if (
			err !== undefined &&
			(Utils.IsUnauthorized(err) ||
				Utils.IsOffline(err) ||
				Utils.IsProxyIssue(err))
		) {
						...
						if (!creds || !creds.CredentialHandler) {
							...
							try {
								...
								this._serverContext.RepoInfo =
									await repositoryInfoClient.GetRepositoryInfo();
	...
	private shouldDisplayTfvcError(errorCode: string): boolean {
		if (
			TfvcErrorCodes.MinVersionWarning === errorCode ||
			...
			fsw.onDidChange(async (uri) => {
				...
				if (remote === undefined) {
					...
					if (this._repoContext.RemoteUrl !== undefined) {
						...
						if (
							remote.toLowerCase() !==
							this._repoContext.RemoteUrl.toLowerCase()
						) {
