...
export class GitClient extends BaseClient {
	...
	public async GetMyPullRequests(): Promise<void> {
		...
		try {
			const request: BaseQuickPickItem = await window.showQuickPick(
			...
			if (request) {
				...
				if (request.id !== undefined) {
					...
					discUrl = GitVcService.GetPullRequestsUrl(
						this._serverContext.RepoInfo.RepositoryUrl,
					);
				}
			...
			this.handleError(
				err,
				GitClient.GetOfflinePullRequestStatusText(),
				false,
				"Error selecting pull request from QuickPick",
			);
		}
	}
	...
	public OpenFileHistory(context: IRepositoryContext): void {
		...
		if (!editor) {
			...
			historyUrl = GitVcService.GetRepositoryHistoryUrl(
				context.RemoteUrl,
				context.CurrentBranch,
			);
	...
	public OpenNewPullRequest(remoteUrl: string, currentBranch: string): void {
		...
		const url: string = GitVcService.GetCreatePullRequestUrl(
			remoteUrl,
			currentBranch,
		);
	...
	public async PollMyPullRequests(): Promise<void> {
		try {
			...
			this.handleError(
				...
				GitClient.GetOfflinePullRequestStatusText(),
				true,
				"Attempting to poll my pull requests",
			);
		}
	}
	...
	private async getMyPullRequests(): Promise<BaseQuickPickItem[]> {
		...
		const myReviewPullRequests: GitPullRequest[] =
			await svc.GetPullRequests(
				this._serverContext.RepoInfo.RepositoryId,
				undefined,
				this._serverContext.UserInfo.Id,
				PullRequestStatus.Active,
	...
	private ensureGitContext(context: IRepositoryContext): void {
		if (context.Type !== RepositoryType.GIT) {
			throw new Error(
				"context sent to GitClient is not a Git context object.",
			);
		}
	}
}
