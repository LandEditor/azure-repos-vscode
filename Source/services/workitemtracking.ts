...
export class WorkItemTrackingService {
	...
	public async CreateWorkItem(
		...
		return await this._witApi.createWorkItem(
			null,
			newWorkItem,
	...
	public async GetWorkItemTypes(
		...
		const filteredTypes: WorkItemType[] = workItemTypes.filter(function (
			el,
		) {
	...
	private async execWorkItemQuery(
		...
		/* tslint:disable:no-null-keyword */
		const workItems: WorkItem[] = await this._witApi.getWorkItems(
			workItemIds,
			[
				WorkItemFields.Id,
	...
	public static GetNewWorkItemUrl(
		teamProjectUrl: string,
		issueType: string,
		title?: string,
		assignedTo?: string,
