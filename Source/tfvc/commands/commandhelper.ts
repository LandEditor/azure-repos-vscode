...
export class CommandHelper {
	...
	public static ProcessErrors(result: IExecutionResult): void {
		if (result.exitCode) {
			...
			if (/Authentication failed/.test(result.stderr)) {
				...
				tfvcErrorCode = TfvcErrorCodes.NotFound;
				message = `${
					Strings.TfInitializeFailureError
				} (${Utils.FormatMessage(result.stdout)})`;
			} else if (
				...
				/TF30063: You are not authorized to access/i.test(result.stderr)
			) {
				...
				messageOptions = [
					{
						...
						telemetryId:
							TfvcTelemetryEvents.ClcCannotAccessWorkspace,
					},
				];
			}
	...
	public static GetFilePath(
		...
		if (
			filePath &&
			filePath.length > 0 &&
			filePath.endsWith(":", filePath.length)
		) {
