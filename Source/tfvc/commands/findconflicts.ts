...
export class FindConflicts implements ITfvcCommand<IConflict[]> {
	...
	public async ParseOutput(
		...
		if (outputToProcess?.includes("_JAVA_OPTIONS")) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (colonIndex >= 0) {
				...
				if (
					/You have a conflicting pending change/i.test(line) ||
					...
					/The item name and content have changed/i.test(line)
				) {
					type = ConflictType.NAME_AND_CONTENT;
				} else if (/The item name has changed/i.test(line)) {
					type = ConflictType.RENAME;
				} else if (
					/The source and target both have changes/i.test(line)
				) {
					...
					/The item has been deleted in the target branch/i.test(line)
				) {
