...
export class TfvcSCMProvider {
	...
	public static GetCheckinInfo(): ICheckinInfo {
		const tfvcProvider: TfvcSCMProvider =
			TfvcSCMProvider.getProviderInstance();
	...
	public async Initialize(): Promise<void> {
		...
		this._sourceControl.acceptInputCommand = {
			command: TfvcCommandNames.Checkin,
			title: "Checkin",
		};
	...
	public static GetLeftResource(resource: Resource): Uri {
		if (
			resource.HasStatus(Status.CONFLICT) ||
	...
	public static GetRightResource(resource: Resource): Uri {
		if (resource.HasStatus(Status.DELETE)) {
			...
			return resource.resourceUri.with({
				query: `C${resource.PendingChange.version}`,
			});
		}
	}
