...
describe("TeamServicesClient-Integration", function () {
	...
	it("should verify repositoryClient.getVstsInfo", async function () {
		...
		assert.isNotNull(
			repoInfo,
			"repoInfo was null when it shouldn't have been"
		);
	...
	it("should verify repositoryClient.getVstsInfo and 404", async function () {
		...
		try {
			const repositoryClient: TeamServicesApi = new TeamServicesApi(
				TestSettings.RemoteRepositoryUrl + "1",
				[CredentialManager.GetCredentialHandler()]
			);
			const repoInfo: any = await repositoryClient.getVstsInfo();
	...
	it("should verify accountClient.connect", async function () {
		...
		const accountClient: TeamServicesApi = new TeamServicesApi(
			TestSettings.AccountUrl,
			[CredentialManager.GetCredentialHandler()]
		);
		...
		assert.isNotNull(
			settings,
			"settings was null when it shouldn't have been"
		);
	...
	it("should verify repositoryClient.validateTfvcCollectionUrl", async function () {
		...
		try {
			const repositoryClient: TeamServicesApi = new TeamServicesApi(
				TestSettings.RemoteTfvcRepositoryUrl,
				[CredentialManager.GetCredentialHandler()]
			);
	...
	it("should verify repositoryClient.validateTfvcCollectionUrl and 404", async function () {
		...
		try {
			const repositoryClient: TeamServicesApi = new TeamServicesApi(
				TestSettings.RemoteTfvcRepositoryUrl + "1",
				[CredentialManager.GetCredentialHandler()]
			);
			await repositoryClient.validateTfvcCollectionUrl();
			assert.fail(
				undefined,
				undefined,
				"validateTfvcCollectionUrl should have thrown but didn't."
