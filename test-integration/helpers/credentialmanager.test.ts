...
describe("CredentialManager-Integration", function () {
	...
	it("should verify store, get, remove credentials for Azure DevOps Services (no token in settings)", async function () {
		try {
			...
			let credInfo: CredentialInfo =
				await credentialManager.GetCredentials(ctx);
	...
	it("should verify store, get, remove credentials for Team Foundation Server", async function () {
		try {
			const ctx: TeamServerContext = Mocks.TeamServerContext(
				"http://java-tfs2015:8081/tfs/DefaultCollection/_git/GitJava"
			);
	...
	it("should verify azure accounts use both host + account", async function () {
		try {
			const ctxAccount1: TeamServerContext = Mocks.TeamServerContext(
				"http://mytest.azure.com/account1/_git/GitJava"
			);
			const ctxAccount2: TeamServerContext = Mocks.TeamServerContext(
				"http://mytest.azure.com/account2/_git/GitJava"
			);
			await credentialManager.StoreCredentials(
				ctxAccount1,
				TestSettings.AccountUser,
			...
			let credInfo: CredentialInfo =
				await credentialManager.GetCredentials(ctxAccount1);
			//For PATs, username stored with StoreCredentials doesn't matter (it's always returned as OAuth)
			assert.equal(credInfo.Username, Constants.OAuth);
			assert.equal(credInfo.Password, TestSettings.Password);
	...
	it("should verify visualstudio accounts only use host", async function () {
		try {
			const ctxAccount1: TeamServerContext = Mocks.TeamServerContext(
				"http://account.visualstudio.com/DefaultCollection/_git/GitJava"
			);
			const ctxAccount2: TeamServerContext = Mocks.TeamServerContext(
				"http://account.visualstudio.com/DefaultCollection/_git/GitJava"
			);
