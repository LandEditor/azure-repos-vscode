...
describe("GitVcService-Integration", function () {
	...
	it("should verify GitVcService.GetRepositories", async function () {
		...
		const repos: GitRepository[] = await svc.GetRepositories(
	...
	it("should verify GitVcService.GetPullRequests", async function () {
		...
		const svc: GitVcService = new GitVcService(ctx);
		const requests: GitPullRequest[] = await svc.GetPullRequests(
			ctx.RepoInfo.RepositoryId
		);
		assert.isNotNull(
			requests,
			"requests was null when it shouldn't have been"
		);
	...
	it("should verify GitVcService.GetPullRequestScore", async function () {
		...
		requests.forEach((request) => {
			totals.push({
				...
				"score": GitVcService.GetPullRequestScore(request),
			});
		});
		...
		for (let index = 0; index < totals.length; index++) {
			...
			if (element.id === 5) {
				assert.equal(element.score, PullRequestScore.Succeeded);
				continue;
			}
			if (element.id === 4) {
				assert.equal(element.score, PullRequestScore.Waiting);
				continue;
			}
			if (element.id === 3) {
				assert.equal(element.score, PullRequestScore.Failed);
				continue;
			}
			if (element.id === 2) {
				assert.equal(element.score, PullRequestScore.NoResponse);
				continue;
			} else {
				//We got a PR we didn't expect but length\count was still the same.
				assert.isTrue(
					false,
					"Expected count of pull requests is incorrect."
				);
			}
		}
	});
