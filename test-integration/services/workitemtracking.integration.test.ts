...
} from "../../src/services/workitemtracking";
...
describe("WorkItemTrackingService-Integration", function () {
	...
	const ctx: TeamServerContext = Mocks.TeamServerContext(
		TestSettings.RemoteRepositoryUrl
	);
	...
	it("should verify WorkItemTrackingService.CreateWorkItem", async function () {
		...
		const title: string =
			"Work item created by integration test (" +
	...
	it("should verify WorkItemTrackingService.GetWorkItems", async function () {
		...
		const svc: WorkItemTrackingService = new WorkItemTrackingService(ctx);
		const items: SimpleWorkItem[] = await svc.GetWorkItems(
			TestSettings.TeamProject,
			WitQueries.MyWorkItems
		);
	...
	it("should verify WorkItemTrackingService.GetQueryResultCount", async function () {
		...
		const svc: WorkItemTrackingService = new WorkItemTrackingService(ctx);
		const query: QueryHierarchyItem = await svc.GetWorkItemQuery(
			TestSettings.TeamProject,
			TestSettings.WorkItemQueryPath
		);
		assert.isNotNull(query);
		//console.log(query);
		expect(query.id).to.equal(TestSettings.WorkItemQueryId);
		const count: number = await svc.GetQueryResultCount(
			TestSettings.TeamProject,
			query.wiql
		);
	...
	it("should verify WorkItemTrackingService.GetWorkItemHierarchyItems", async function () {
		...
		const svc: WorkItemTrackingService = new WorkItemTrackingService(ctx);
		const items: QueryHierarchyItem[] = await svc.GetWorkItemHierarchyItems(
			TestSettings.TeamProject
		);
	...
	it("should verify WorkItemTrackingService.GetWorkItemTypes", async function () {
		...
		const svc: WorkItemTrackingService = new WorkItemTrackingService(ctx);
		const items: WorkItemType[] = await svc.GetWorkItemTypes(
			TestSettings.TeamProject
		);
		assert.isNotNull(items);
		//console.log(items.length);
	...
	it("should verify WorkItemTrackingService.GetWorkItemById", async function () {
		...
		const svc: WorkItemTrackingService = new WorkItemTrackingService(ctx);
		const item: SimpleWorkItem = await svc.GetWorkItemById(
			TestSettings.WorkItemId.toString()
		);
	...
	it("should verify WorkItemTrackingService.GetWorkItemQuery with a Link query", function (done) {
		...
		const svc: WorkItemTrackingService = new WorkItemTrackingService(ctx);
		svc.GetWorkItemQuery(
			TestSettings.TeamProject,
			TestSettings.WorkItemLinkQueryPath
		).then(
			function (query) {
				assert.isNotNull(query);
				//console.log(query);
				svc.GetWorkItems(TestSettings.TeamProject, query.wiql)
					.then((items) => {
						assert.isTrue(
							items.length > 0,
							"Expected at least 1 result but didn't get any."
						);
	...
	it("should verify WorkItemTrackingService.GetWorkItemQuery with maximum 200 results", function (done) {
		...
		svc.GetWorkItemQuery(
			TestSettings.TeamProject,
			TestSettings.WorkItemTwoHundredTasksQueryPath
		).then(
			function (query) {
				assert.isNotNull(query);
				//console.log(query);
				svc.GetWorkItems(TestSettings.TeamProject, query.wiql)
					.then((items) => {
						...
						assert.isTrue(
							items.length === 200,
							"Expected the maximum of 200 work items but didn't get that amount."
						);
						done();
					})
					.catch((err) => {
	...
	it("should verify WorkItemTrackingService.GetWorkItemQuery which returns zero results", function (done) {
		...
		svc.GetWorkItemQuery(
			TestSettings.TeamProject,
			TestSettings.WorkItemZeroResultsQueryPath
		).then(
			function (query) {
				assert.isNotNull(query);
				//console.log(query);
				svc.GetWorkItems(TestSettings.TeamProject, query.wiql)
					.then((items) => {
						assert.isTrue(
							items.length === 0,
							"Expected zero results but actually got some."
						);
						//assert.isTrue(items.length === 200, "Expected the maximum of 200 work items but didn't get that amount.");  // current maximum work items returned
						done();
					})
